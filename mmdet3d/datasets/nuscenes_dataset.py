...
class NuScenesDataset(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        input_dict = self.fix_data_path(input_dict)   
        return input_dict

    def fix_data_path(self, input_dict):
        if self.data_root not in input_dict['pts_filename']:
            pts_filename = input_dict['pts_filename'].split('/samples/LIDAR_TOP/')[1]
            input_dict['pts_filename'] = os.path.join(self.data_root, 'samples', 'LIDAR_TOP', pts_filename)
            if 'sweeps' in input_dict:
                for sweep in range(len(input_dict['sweeps'])):
                    if 'sweeps' in input_dict['sweeps'][sweep]['data_path']:
                        sweep_path = input_dict['sweeps'][sweep]['data_path'].split('/sweeps/LIDAR_TOP/')[1]
                        input_dict['sweeps'][sweep]['data_path'] = os.path.join(self.data_root, 'sweeps', 'LIDAR_TOP', sweep_path)  
                    else:
                        sweep_path = input_dict['sweeps'][sweep]['data_path'].split('/samples/LIDAR_TOP/')[1]
                        input_dict['sweeps'][sweep]['data_path'] = os.path.join(self.data_root, 'samples', 'LIDAR_TOP', sweep_path)                  

            if 'curr' in input_dict:
                input_dict['curr'] = self.fix_data_path_func(input_dict['curr'])
                
            if 'adjacent' in input_dict:
                for i in range(len(input_dict['adjacent'])):
                    input_dict['adjacent'][i] = self.fix_data_path_func(input_dict['adjacent'][i])
        return input_dict
      
    def fix_data_path_func(self, input_dict):
        lidar_path = input_dict['lidar_path'].split('/samples/LIDAR_TOP/')[1]
        input_dict['lidar_path'] = os.path.join(self.data_root, 'samples', 'LIDAR_TOP', lidar_path)
        if 'sweeps' in input_dict:
            for sweep in range(len(input_dict['sweeps'])):
                if 'sweeps' in input_dict['sweeps'][sweep]['data_path']:
                    sweep_path = input_dict['sweeps'][sweep]['data_path'].split('/sweeps/LIDAR_TOP/')[1]
                    ...
                    sweep_path = input_dict['sweeps'][sweep]['data_path'].split('/samples/LIDAR_TOP/')[1]
                    input_dict['sweeps'][sweep]['data_path'] = os.path.join(self.data_root, 'samples', 'LIDAR_TOP', sweep_path)  
        if 'cams' in input_dict:
            for cam in input_dict['cams']:
                cam_path = input_dict['cams'][cam]['data_path'].split(f'/samples/{cam}/')[1]
                input_dict['cams'][cam]['data_path'] = os.path.join(self.data_root, 'samples', cam, cam_path)     
        return input_dict
