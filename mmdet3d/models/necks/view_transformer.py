...
class LSSViewTransformer(BaseModule):
    ...
    def voxel_pooling_v2(self, coor, depth, feat):
        ...
        if ranks_feat is None:
            warnings.warn(
                'No points fall within the configured BEV receptive field.')
    ...
    def voxel_pooling_prepare_v2(self, coor):
        ...
        ranks_depth = torch.arange(
            num_points, dtype=torch.int, device=coor.device)
        ranks_feat = torch.arange(
            num_points // D, dtype=torch.int, device=coor.device)
        ...
        batch_idx = torch.arange(B, device=coor.device).reshape(B, 1)
        batch_idx = batch_idx.expand(B, num_points // B).reshape(
            num_points, 1)
...
class DepthLightNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 bias=0.0,
                 aspp_mid_channels=-1):
        super(DepthLightNet, self).__init__()
        self.reduce_conv = nn.Sequential(
            nn.Conv2d(
                in_channels, mid_channels, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(mid_channels),
        ...
        self.depth_mlp = Mlp(10, mid_channels, mid_channels)
        self.depth_se = SELayer(mid_channels)  # NOTE: add camera-aware
        self.context_mlp = Mlp(10, mid_channels, mid_channels)
        self.context_se = SELayer(mid_channels)  # NOTE: add camera-aware
        depth_conv_input_channels = mid_channels
        downsample = None
...
@NECKS.register_module()
class LSSViewTransformerSphericalDepth(BaseModule):

    def __init__(self, in_channels, out_channels, loss_depth_weight=3.0, depthnet_cfg=dict(), downsample=4,  **kwargs):
        super(LSSViewTransformerSphericalDepth, self).__init__(**kwargs)
        self.loss_depth_weight = loss_depth_weight
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.D = 1  # only one depth channel for spherical projection
        self.depth_net = DepthLightNet(self.in_channels, self.in_channels,
                                  self.out_channels, self.D, **depthnet_cfg)
        self.downsample = downsample

    def get_mlp_input(self, intrin, post_rot, post_tran):
        """
        Args:
            intrin: (B, N, 3, 3)
            post_rot: (B, N, 3, 3)
    ...
    def get_gt_depth(self, gt_depths):
        ...
        Input:
            gt_depths: [B, N, H, W]
        Output:
            downsampled_gt_depths: [B*N, H//ds, W//ds]
        """
        B, N, H, W = gt_depths.shape
        gt_depths = gt_depths.view(B * N, 1, H, W)  # add channel dim
        gt_depths = F.interpolate(gt_depths, scale_factor=1.0 / self.downsample, mode='nearest')
        gt_depths = gt_depths.squeeze(1)  # shape: [B*N, H//ds, W//ds]
        return gt_depths

    @force_fp32()
    def get_depth_loss(self, depth_labels, depth_preds):
        depth_labels = self.get_gt_depth(depth_labels)
        depth_preds = depth_preds[:, 0]  # assuming shape [B*N, 1, H, W]
        mask = depth_labels > 0
        loss = F.l1_loss(depth_preds[mask], depth_labels[mask], reduction='mean')
        return self.loss_depth_weight * loss

    def forward(self, input, stereo_metas=None):
        (x, rots, trans, intrins, post_rots, post_trans, mlp_input) = input
    ...
    # def pre_compute(self, input):
    #     if self.initial_flag:
    #         coor = self.get_lidar_coor(*input[1:6])
    #         self.init_acceleration_v2(coor)
    #         self.initial_flag = False
    def view_transform_core(self, input, depth, tran_feat):
        B, N, C, H, W = input[0].shape
