...
class Mask2FormerNuscOccHead(MaskFormerHead):
    ...
    def get_targets(self, cls_scores_list, mask_preds_list, gt_labels_list,
                    gt_masks_list, gt_lidarseg_list, img_metas, visible_masks):
        ...
        (labels_list, label_weights_list, mask_targets_list, mask_weights_list,
         ...
         neg_inds_list) = multi_apply(self._get_target_single, cls_scores_list, mask_preds_list, 
                    gt_labels_list, gt_masks_list, gt_lidarseg_list, img_metas, visible_masks)
    ...
    def _get_target_single(self, cls_score, mask_pred, gt_labels, gt_masks, 
                    gt_lidarseg, img_metas, visible_mask):
        ...
        # lidar2ego = torch.from_numpy(img_metas['lidar2ego']).to(gt_lidarseg)
        ...
        visible_points_mask = point_sample_3d(visible_mask.unsqueeze(0).unsqueeze(1).float(), 
            point_coords.repeat(1, 1, 1), padding_mode=self.padding_mode).squeeze(1)

        visible_points_mask = visible_points_mask > 0.9
        mask_points_pred = mask_points_pred[:, visible_points_mask[0]]
        gt_points_masks = gt_points_masks[:, visible_points_mask[0]]
    ...
    def loss(self, all_cls_scores, all_mask_preds, gt_labels_list,
        ...
        visible_masks_list = [visible_masks for _ in range(num_dec_layers)]
        
        losses_cls, losses_mask, losses_dice = multi_apply(
            self.loss_single, all_cls_scores, all_mask_preds,
            all_gt_labels_list, all_gt_masks_list, all_gt_lidarseg_list, 
            img_metas_list, visible_masks_list)
    ...
    def loss_single(self, cls_scores, mask_preds, gt_labels_list,
        ...
        # use visible mask
        num_gt = mask_point_targets.shape[0]
        visible_masks = visible_masks.unsqueeze(1).repeat(num_gt, 1, 1, 1, 1).float()
        visible_points_mask = point_sample_3d(visible_masks, 
            point_coords, padding_mode=self.padding_mode).squeeze(1)
        visible_points_mask = visible_points_mask > 0.9
        mask_point_preds = mask_point_preds[:, visible_points_mask[0]]
        mask_point_targets = mask_point_targets[:, visible_points_mask[0]]
    ...
    def forward_train(self,
        ...
        losses = self.loss(all_cls_scores, all_mask_preds, gt_labels, 
                           gt_masks, points, img_metas, visible_mask)
    ...
    def simple_test(self, 
        ...
        mask_pred_results = F.interpolate(
            mask_pred_results,
            size=tuple(self.test_cfg.occ_size),
            mode='trilinear',
