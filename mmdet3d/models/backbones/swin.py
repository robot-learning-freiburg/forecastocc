...
class PatchMergingAssymetrical(BaseModule):
    """Merge patch feature map.
    ...
    def __init__(self,
        ...
        sample_dim = stride[1] * stride[0] * in_channels
    ...
    def forward(self, x, hw_shape):
        ...
        # stride is fixed to be equal to kernel_size.
        if (H % self.stride[0] != 0) or (W % self.stride[1] != 0):
            x = F.pad(x, (0, W % self.stride[1], 0, H % self.stride[0]))
        ...
        x = self.reduction(x)
        if self.stride[0] == 1:
            down_hw_shape = H , (W + 1) // self.stride[1]
        else:
            down_hw_shape = (H + 1) // self.stride[0], (W + 1) // self.stride[1]
        return x, down_hw_shape
...
@BACKBONES.register_module()
class SwinTransformerAssymetric(BaseModule):
    """ Swin Transformer
    A PyTorch implement of : `Swin Transformer:
    Hierarchical Vision Transformer using Shifted Windows`  -
        https://arxiv.org/abs/2103.14030
    ...
    def __init__(self,
                 ...
                 output_missing_index_as_none=False,
                 frozen_stages=-1):
        super(SwinTransformerAssymetric, self).__init__()
        ...
        assert strides[0][1] == patch_size, 'Use non-overlapping patch embed.'
        ...
        if self.use_abs_pos_embed:
            patch_row = pretrain_img_size[0] // 1
            patch_col = pretrain_img_size[1] // patch_size
            num_patches = patch_row * patch_col
            self.absolute_pos_embed = nn.Parameter(
                torch.zeros((1, num_patches, embed_dims)))
        ...
        for i in range(num_layers):
            if i < num_layers - 1:
                downsample = PatchMergingAssymetrical(
                    in_channels=in_channels,
                    out_channels=2 * in_channels,
                    stride=(strides[i + 1][0], strides[i + 1][1]),
                    norm_cfg=norm_cfg if patch_norm else None,
                    init_cfg=None)
            else:
                downsample = None
            ...
            for table_key in relative_position_bias_table_keys:
                ...
                if nH1 != nH2:
                    ...
                    if L1 != L2:
                        ...
                        state_dict[table_key] = table_pretrained_resized.view(
                            nH2, L2).permute(1, 0).contiguous()
            state_dict.pop('patch_embed.projection.weight', None)
            for i in range(0, 3):
                state_dict.pop(f'stages.{i}.downsample.norm.weight', None)
                state_dict.pop(f'stages.{i}.downsample.norm.bias', None)
                state_dict.pop(f'stages.{i}.downsample.reduction.weight', None)
    ...
    def train(self, mode=True):
        """Convert the model into training mode while keep normalization layer
        freezed."""
        super(SwinTransformerAssymetric, self).train(mode)
        self._freeze_stages()