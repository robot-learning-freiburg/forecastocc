...
class COTR(BEVStereo4D):
    ...
    def __init__(self,
        ...
        if freeze_blocks:
            # print('Freeze blocks in img_backbone')
        ...
        if freeze_patch_embed:
            # print('Freeze patch_embed in img_backbone')
            for name, module in self.img_backbone.named_modules():
    ...
    def image_encoder(self, img, stereo=False):
        ...
        assert self.with_img_neck, \
            'COTR requires a neck in image branch'
    ...
    def extract_stereo_ref_feat(self, x):
        ...
        if isinstance(self.img_backbone,ResNet):
            ...
            self.img_backbone.__class__.__name__ == 'BEiTAdapterCP':
    ...
    def simple_test(self,
        ...
        if len(img_metas) > 1:
            occ_results = [{'occ': occ['occ'][i]} for i in range(len(img_metas))]
        else:
            occ_results = [{'occ': occ['occ']}]
