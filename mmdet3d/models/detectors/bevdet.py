...
class BEVDet(CenterPoint):
    ...
    def image_encoder(self, img, stereo=False):
        ...
        assert self.with_img_neck, \
            'BEVDet requires a neck in image branch'
...
@DETECTORS.register_module()
class BEVDepth4DAdap(BEVDepth4D):
    def __init__(self, freeze_blocks=False, freeze_patch_embed=False, **kwargs):
        super(BEVDepth4DAdap, self).__init__(**kwargs)            
        if freeze_blocks:
            for name, module in self.img_backbone.named_modules():
                if 'blocks' in name:
    ...
    def extract_stereo_ref_feat(self, x):
        ...
        if isinstance(self.img_backbone,ResNet):
            ...
            for i, layer_name in enumerate(self.img_backbone.res_layers):
                ...
                x = res_layer(x)
                return x
        elif self.img_backbone.__class__.__name__ == 'BEiTAdapter' or \
            self.img_backbone.__class__.__name__ == 'BEiTAdapterCP' or \
            self.img_backbone.__class__.__name__ == 'EfficientNet':
                x = self.img_backbone(x)
                return x[0]
        else:
            print (f'Warning: {self.img_backbone.__class__.__name__} is not supported')
...
@DETECTORS.register_module()
class BEVStereo4DAdap(BEVStereo4D):
    def __init__(self, freeze_blocks=False, freeze_patch_embed=False, **kwargs):
        super(BEVStereo4D, self).__init__(**kwargs)
        self.extra_ref_frames = 1
        self.temporal_frame = self.num_frame
